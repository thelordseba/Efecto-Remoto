  ...
  const handleClickReset = async () => {    
    if(password.confirmPass === password.pass){
      try {      
        ...
        await axios.put(`${process.env.REACT_APP_API}/users/${user.id}`, {email: user.email});  //FALTA COMPLETAR LA RUTA
    ...
    <> 
      <div className="resetPass-container">
        <div className="container-summary">          
          ...
          <div><input name="pass" type="password" onChange={handleOnChange}></input></div>
          <div className="text1"> Repetir nueva contraseña: </div>
          <div><input name="confirmPass" type="password" onChange={handleOnChange}></input></div>
          {error?(<div className="errorMessage">Comprueba tu contraseña.</div>) : null}
